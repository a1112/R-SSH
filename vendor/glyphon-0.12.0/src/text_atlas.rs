...
/// Physical texture allocation retained by a [`TextAtlas`].
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct TextAtlasMetrics {
...
impl TextAtlas {
    ...
    /// Creates an atlas whose two physical textures may not exceed
    /// `max_bytes` in total.
    pub fn with_max_bytes(
        ...
        let initial_dimension =
            InnerAtlas::INITIAL_SIZE.min(device.limits().max_texture_dimension_2d) as usize;
        let initial_bytes = initial_dimension
            .saturating_mul(initial_dimension)
            .saturating_mul(5);
        if initial_bytes > max_bytes {
            return None;
        }
        let mut atlas = Self::new(device, queue, cache, format);
        atlas.max_bytes = Some(max_bytes);
        Some(atlas)
    }

    /// Changes the physical texture cap. Returns `false` when the current
    /// allocation already exceeds the requested cap.
    pub fn set_max_bytes(&mut self, max_bytes: usize) -> bool {
        if self.metrics().allocated_bytes > max_bytes {
            return false;
        }
    ...
    /// Returns actual allocated texture dimensions and bytes.
    pub fn metrics(&self) -> TextAtlasMetrics {
        let mask_dimension = self.mask_atlas.size;
        let color_dimension = self.color_atlas.size;
        let mask_bytes = mask_dimension as usize * mask_dimension as usize;
        let color_bytes = color_dimension as usize * color_dimension as usize * 4;
        TextAtlasMetrics {
            mask_dimension,
            color_dimension,
            allocated_bytes: mask_bytes.saturating_add(color_bytes),
    ...
    pub(crate) fn grow(
        ...
        if let Some(max_bytes) = self.max_bytes {
            ...
            let next = inner
                .size
                .saturating_mul(2)
                .min(inner.max_texture_dimension_2d);
            let next_bytes = match content_type {
                ContentType::Mask => {
                    next as usize * next as usize
                        + current.color_dimension as usize * current.color_dimension as usize * 4
                }
                ContentType::Color => {
                    current.mask_dimension as usize * current.mask_dimension as usize
                        + next as usize * next as usize * 4
                }
            };
