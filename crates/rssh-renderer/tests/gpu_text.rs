...
fn id_exhaustion_retries_full_frame_without_aliasing_and_failure_is_atomic() {
    ...
    recoverable
        .enable_text(
            catalog(),
            font_config(),
            config(4 * 1024 * 1024).with_identifier_limit_for_tests(2),
        )
    ...
    let mut atomic = GpuLayerRenderer::new(&context, wgpu::TextureFormat::Rgba8Unorm, 64 * 1024)
        .expect("atomic renderer");
    atomic
        .enable_text(
            catalog(),
            font_config(),
            config(4 * 1024 * 1024).with_identifier_limit_for_tests(1),
        )
