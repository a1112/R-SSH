...
- `rssh-app doctor` reports the selected PTY backend, terminal size, and child
  terminal environment, and checks that the local default shell and the OpenSSH
  command-line tools used by the console-backed SSH, SFTP, and SCP paths are
  available.
...
- `rssh-app local --metrics`, `ssh --metrics`, `sftp --metrics`, and
  ...
  code, signal, success state, final session state, PTY input/output bytes,
...
- `rssh-app profile --list --verbose --file PATH` also prints each profile's
  resolved startup command line for faster console-side scanning.
- `rssh-app profile --list --json --file PATH` prints machine-readable profile
  entries with name, kind, resolved command line, and argv.
