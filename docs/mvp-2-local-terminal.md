...
- The app answers DEC request status string queries (`DECRQSS`,
  ...
  underline, strikethrough, inverse video, and colors; cursor shape (`SP q`);
  and scrolling region (`r`), preserving ST versus C1 ST response terminators
  and returning an invalid status for unsupported selectors.
...
- DECRQSS response: unit tests cover current SGR, including faint text,
  cursor-shape, and scroll-region status queries in both DCS and C1 DCS forms.
