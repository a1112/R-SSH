...
- `rssh-app` counts ASCII BEL events from active and inactive pane output and
  ...
  `config.colors.foreground`, `config.colors.background`, and
  ...
  `colors.visual_bell`; native `foreground_color` and `background_color`
  overrides stand in for WezTerm `colors.foreground` and `colors.background`,
  driving the default text foreground and framebuffer background for full and
...
- `rssh-app` dispatches a typed native-window `window-config-reloaded` hook for
  ...
  `disable_default_quick_select_patterns`, `quick_select_remove_styling`, `selection_word_boundary`, `term`, `audible_bell`, `visual_bell`, `foreground_color`, `background_color`, `visual_bell_color`, `notification_handling`, `default_prog`,
