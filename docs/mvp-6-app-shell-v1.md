...
- `rssh-app` counts ASCII BEL events from active and inactive pane output and
  ...
  Lua table variables, and static `config.color_schemes['Name'] = scheme`
  assignments can append or replace named schemes after initialization. `config.color_scheme`
