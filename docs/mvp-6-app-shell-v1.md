...
- `rssh-app` dispatches a typed native-window `format-tab-title` hook when
  ...
  Lua callbacks and the full Lua config object remain later parity work.
...
- `rssh-app` dispatches typed native-window `update-status` and deprecated
  ...
  into the same status state, including static `wezterm.format` Text-item
  visible-text composition. Arbitrary Lua callbacks and styled `wezterm.format`
  SGR composition remain later parity work.
