...
- `rssh-app` includes a quick-select overlay (`Ctrl+Shift+Space`) and
  ...
  selected quick-select match through the active pane, and callbacks that store
  `window:get_selection_text_for_pane(pane)` in a local variable and pass it to
  `wezterm.open_with(...)` dispatch that match through the native open-uri hook;
  arbitrary custom callback execution remains open.
- `rssh-app` exposes a native WezTerm-style `PromptInputLine` action payload
  ...
  Static callback bodies that call `pane:send_text(line)` map submitted text to
  the native `SendString` path, while `pane:send_paste(line)` maps submitted
  text to the native `SendPaste` path with paste newline and bracketed-paste
  encoding; arbitrary Lua `wezterm.action_callback` execution remains later
  parity work.
...
- `rssh-app` exposes a native WezTerm-style `EmitEvent` action payload carrying
  ...
  map matching `EmitEvent` names onto existing native commands in order after
  the typed native event dispatch, and top-level
  ...
  Arbitrary Lua `wezterm.on`/`wezterm.emit` wiring remains later parity work.
...
- `rssh-app` counts ASCII BEL events from active and inactive pane output and
  ...
  overriding fields. Static `wezterm.color.load_scheme(...)` paths may be
  inline quoted or long-bracket literals, or statically evaluable top-level
  string expressions composed of quoted/long-bracket literals, identifier
  variable chains, and pure `..` concatenation. Each identifier resolves to its
  latest top-level binding at that evaluation point; an assigned variable
  evaluates its right-hand side at the assignment offset, while the call
  argument evaluates at the original call offset, preserving assignment-time
  capture and isolating the loaded path from rebinding after the call. One
  resolver is shared by `config.colors` and selected
  `config.color_schemes['Name']` entries and normalizes canonical direct calls,
  module aliases, direct or parenthesized `require('wezterm')` receivers,
  static-key access, and function aliases, including supported Lua comments
  between helper-path segments or before call arguments. Loaded colors can flow
  directly, through static variables whose supported mutations are applied, or
  ...
  `wezterm.config_dir` (including `local wt = require 'wezterm'` alias forms) now
  resolves from `WEZTERM_CONFIG_DIR` in static path composition.
...
- `rssh-app` dispatches typed native-window `update-status` and deprecated
  ...
  including dynamic variable fallbacks such as `title or ''` or
  `title or fallback` where `fallback` is a callback-local or top-level static
  string variable, variable fallback concat segments such as `(title or '')` or
  `(title or fallback)`, and direct dynamic
  method fallbacks such as `window:active_tab():get_title() or ''` or
  `tab:get_title() or ''`, including parenthesized fallback concat segments,
  ...
  `local item_index = 1; local env = window:effective_config().launch_menu[item_index].set_environment_variables; env[env_key]`. The documented
...
- The tab bar can hide tab indices via the native
  ...
  `colors.tab_bar` item styles parse `fg_color`/`bg_color`, intensity, italic,
  strikethrough, and underline values inline or through top-level static value
  variables, including underline
