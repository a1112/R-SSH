...
- Incomplete CSI, OSC title, DCS, and `ESC7`/`ESC8` sequences are retained
  across `Terminal::feed` calls so PTY chunk boundaries do not leak control
  bytes into the grid.
...
- DCS control strings terminated by ST are ignored so unsupported terminal
  capability probes do not appear as terminal text.
- Basic SGR handling:
  - reset
...
- split CSI, OSC title, DCS, and `ESC7`/`ESC8` sequences across `feed` calls
...
- DCS control-string filtering with split-sequence buffering
