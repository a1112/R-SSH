...
- Native framebuffer rendering applies basic WezTerm-style
  ...
  native window background gradient path. Layer `hsb` tables apply to Color
  sources and explicit Gradient color lists. Multi-layer composition, image
  sources, alignment/repeat settings, HSB transforms for preset/image sources,
  and semi-transparent gradient layers remain future parity work.
