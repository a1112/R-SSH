...
- Native tab bar rendering now dispatches a typed `format-tab-title` hook after
  ...
  simple top-level `if tab.is_active` / `if tab.is_last_active` return branches
  followed by a fallback return,
  ...
  `wezterm.truncate_right(title, max_width - N)` truncation, and local
