...
- Native window now dispatches a typed `new-tab-button-click` hook for
  ...
  `default_action` guard or `default_action ~= nil` guard before returning
  `false` run the native default action exactly once; arbitrary Lua event
