...
- Native window now dispatches typed `update-status` and deprecated
  ...
  static `pane:get_dimensions()` field concatenations for `cols`,
  ...
  active/inactive strings, static `window:get_dimensions()` field
  concatenations for `pixel_width`, `pixel_height`, `dpi`, and
  `is_full_screen`, and the documented
