...
- Native `ActivateKeyTable`, `PopKeyTable`, and `ClearKeyTableStack` action
  ...
  lifecycle reload applies only to the supported strict production grammar.
...
- Native `CopyMode` assignment payloads now parse WezTerm-style
  ...
  file-backed lifecycle reload applies only to the supported strict production
  grammar.
