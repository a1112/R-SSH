...
- Native window now dispatches typed `update-status` and deprecated
  ...
  `window:set_right_status(...)` calls update native tab-bar status text.
  Arbitrary Lua status callback execution,
