...
- Native framebuffer rendering applies basic WezTerm-style
  `window_background_gradient` tables with `Horizontal` or `Vertical`
  orientation and explicit `colors` lists to default window backgrounds, with
  `Vertical` following WezTerm's bottom-to-top direction. Static Lua snippets
  for that subset now parse into native/effective config. Linear angle tables,
  radial gradients, presets, interpolation, blend modes, noise, and segment
