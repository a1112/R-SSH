...
- Native window now dispatches typed `update-status` and deprecated
  ...
  with `window:window_id()` or callback `pane:pane_id()`, static `window:get_dimensions()` field
