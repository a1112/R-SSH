...
- Native framebuffer rendering applies basic WezTerm-style
  ...
  Static single-layer WezTerm `config.background` tables now parse
  `source = { Color = ... }` into the native background color path, preserving
  layer opacity as alpha, and opaque `source = { Gradient = ... }` into the
  native window background gradient path. Layer `hsb` tables apply inline or
  through top-level static table variables to Color sources and explicit
  Gradient color lists. Multi-layer composition, image sources,
