...
- App runtime and console output filtering now track kitty keyboard
  ...
  take priority over that ambiguous-width setting, while
  actual left Ctrl+Alt-to-AltGr input remapping, platform IME/XIM connection,
  preedit rendering, and broader dynamic `cell_widths` Lua parity remain open.
...
- Native user `key_assignments` now dispatch matching regular key presses
  ...
  priority over the ambiguous-width option. Actual left Ctrl+Alt-to-AltGr input
  remapping, platform IME/XIM connection, preedit rendering, broader dynamic
  `cell_widths` Lua parity, and broader kitty alternate-key variants remain
  open.
