...
- Native tab bar rendering now dispatches a typed `format-tab-title` hook after
  ...
  `wezterm.on('format-tab-title', function(...) return ... end)` literal string
  and callback-local/top-level static string-variable returns,
...
- Native window now dispatches a typed `format-window-title` hook after
  ...
  callbacks with literal string returns or callback-local/top-level static
  string-variable returns and concatenation, plus top-level static `local <alias> = wezterm.on` event
