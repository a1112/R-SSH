...
- Native window now dispatches a typed `format-window-title` hook after
  ...
  `tab.active_pane.domain_name`, `tab.tab_title`, callback-local dynamic title
  variables, or callback-local `tab.active_pane`/`pane` active-pane aliases
  resolve the active pane/tab title or domain name at recompute time, including
  `tostring(...)` wrappers;
