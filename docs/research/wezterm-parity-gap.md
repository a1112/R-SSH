...
- Native window now dispatches a typed `window-config-reloaded` hook for
  ...
  `enable_csi_u_key_encoding`, `enable_kitty_keyboard`, `allow_win32_input_mode`, `treat_left_ctrlalt_as_altgr`, `treat_east_asian_ambiguous_width_as_wide`, `normalize_output_to_unicode_nfc`, `unicode_version`, `use_ime`, `ime_preedit_rendering`, `macos_forward_to_ime_modifier_mask`, `xim_im_name`, `scroll_to_bottom_on_input`,
