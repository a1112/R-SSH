...
- Native window now dispatches typed `update-status` and deprecated
  ...
  `window:active_workspace()` status arguments, and the documented
  ...
  status example with static active/inactive strings, plus inline,
  ...
  status callback execution, dynamic `wezterm.format` construction, and
  remaining window status API wiring remain open.
