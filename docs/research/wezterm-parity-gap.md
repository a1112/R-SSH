...
- App runtime and console output filtering now track kitty keyboard
  ...
  `false`, `treat_east_asian_ambiguous_width_as_wide` with WezTerm's default
  `false`, `use_ime` with WezTerm's current default `true`,
  ...
  character width calculation for ambiguous East Asian width characters, while
  actual left Ctrl+Alt-to-AltGr input remapping, platform IME/XIM connection,
  preedit rendering, and `cell_widths` override tables remain open.
...
- Native user `key_assignments` now dispatch matching regular key presses
  ...
  character width calculation for active and new panes; actual left
  Ctrl+Alt-to-AltGr input remapping, platform IME/XIM connection, preedit
  rendering, `cell_widths` override tables, and broader kitty alternate-key
...
- Command palette now exposes WezTerm-style `ReloadConfiguration`, and the
  ...
  actual update checks and update-window UI remain open. `front_end` is retained
