...
- App runtime and console output filtering now track kitty keyboard
  ...
  `xim_im_name`; actual left Ctrl+Alt-to-AltGr input remapping, platform
  IME/XIM connection, and preedit rendering remain open.
...
- Native user `key_assignments` now dispatch matching regular key presses
  ...
  `config.treat_left_ctrlalt_as_altgr`, `config.use_ime`,
  `config.ime_preedit_rendering`, and `config.xim_im_name` now parse into the
  same native override path; actual left Ctrl+Alt-to-AltGr input remapping,
  platform IME/XIM connection, preedit rendering, and broader kitty
  alternate-key variants remain open.
...
- Native window now dispatches a typed `window-config-reloaded` hook for
  ...
  `enable_csi_u_key_encoding`, `enable_kitty_keyboard`, `allow_win32_input_mode`, `treat_left_ctrlalt_as_altgr`, `use_ime`, `ime_preedit_rendering`, `xim_im_name`, `scroll_to_bottom_on_input`,
