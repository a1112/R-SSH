...
- Native `CopyMode` assignment payloads now parse WezTerm-style
  ...
  `NextMatch` and `PriorMatch` assignment values dispatch the same copy-mode
  search stepping helpers as the default key table. Remaining page/clear/cycle,
  edit, and accept CopyModeAssignment values plus SetSelectionMode
  `Word`/`SemanticZone` remain open.
