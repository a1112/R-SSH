...
- Native tab bar rendering now dispatches a typed `format-tab-title` hook after
  ...
  return `tab.tab_title` or `tab.active_pane.title` resolve the current explicit
  tab title or pane title at render time; arbitrary Lua callbacks and the full
  Lua config object remain open.
