...
- Native window now dispatches typed `update-status` and deprecated
  ...
  scope plus callback-local `table.insert` appends. Arbitrary Lua status
  callback execution, dynamic `wezterm.format` construction, and broader window
  status API wiring remain open.
