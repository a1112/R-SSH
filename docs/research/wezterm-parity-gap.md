...
- Native window now dispatches a typed `format-window-title` hook after
  ...
  `tab.active_pane.domain_name`, `tab.active_pane.foreground_process_name`,
  `tab.tab_title`, callback-local dynamic title variables, or callback-local
  `tab.active_pane`/`pane` active-pane aliases resolve the active pane/tab
  title, domain name, or foreground process at recompute time, including
  `tostring(...)` wrappers;
