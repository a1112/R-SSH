...
- Native framebuffer rendering applies basic WezTerm-style
  ...
  cover-style sizing plus the fixed-pixel layout subset for `width`, `height`,
  `repeat_x`, `repeat_y`, `horizontal_align`, and `vertical_align`. Richer
  multi-layer image composition, `Attachment` image sources, `Mirror` repeat
  mode, percentage/cell sizing, offsets, and repeat sizing remain future parity
  work.
