...
- Native window now dispatches a typed `window-config-reloaded` hook for
  ...
  `audible_bell`, `visual_bell`, `colors.foreground`, `colors.background`, `colors.cursor_bg`, `colors.visual_bell`, and
  ...
  `text_background_opacity`, `window_background_opacity`, `colors.foreground`, `colors.background`, and `colors.cursor_bg`, plus
...
- Native cursor rendering applies WezTerm-style `colors.cursor_bg` as the
  default cursor color unless OSC cursor color or `force_reverse_video_cursor`
  takes precedence. Static WezTerm-style Lua `config.colors.cursor_bg`
  snippets now parse into the same native override path; `cursor_fg` and
  `cursor_border` remain later parity work.
