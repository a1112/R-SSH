...
- Native window now dispatches a typed `new-tab-button-click` hook for
  ...
  callback-local aliases assigned from `pane` are passed to `perform_action`;
  arbitrary Lua event execution and dynamic return logic remain open.
