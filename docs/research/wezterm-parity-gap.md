...
- Native window now dispatches a typed `format-window-title` hook after
  ...
  callback variables, top-level static table-field function callbacks, literal
  string returns, callback-local/top-level static string-variable returns and
  concatenation, plus top-level static
