...
- Native window now dispatches typed `update-status` and deprecated
  ...
  `pane:pane_id()`/`pane:get_title()`/`pane:get_domain_name()`, static `window:get_dimensions()` field
