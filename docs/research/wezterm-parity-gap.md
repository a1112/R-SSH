...
- Native window now dispatches a typed `new-tab-button-click` hook for
  ...
  using `button == 'Left'/'Right'/'Middle'` or
  `button ~= 'Left'/'Right'/'Middle'` map onto the same suppression path. The
