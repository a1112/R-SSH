...
- Native `ActivateKeyTable`, `PopKeyTable`, and `ClearKeyTableStack` action
  ...
  Lua snippets for `config.keys` and `config.key_tables` now parse native key
  assignment tables into the same override/runtime path for the implemented
  action subset; full Lua config evaluation, default key-table merging, and
...
- Native `leader` overrides now expose a WezTerm-style `LEADER` modal modifier
  ...
  before normal input resumes. Lua `leader` parsing plus full keys
  config-file wiring remain open.
...
- Native `CopyMode` assignment payloads now parse WezTerm-style
  ...
  static Lua snippets for `config.keys` and `config.key_tables` now parse the
  implemented native assignment subset into runtime key-table overrides; full
