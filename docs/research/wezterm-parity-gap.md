...
- PTY reader events now carry app-shell `WindowId` plus `PaneId`, avoiding
  ...
  `table.insert` appends, through static field names in direct return-table
  configs, or through `table.insert(config.clean_exit_codes, ...)` appends.
