...
- Native window state now exposes WezTerm-style `DecreaseFontSize`,
  ...
  native override path. Renderer font rasterization and full Lua config
...
- App runtime and console output filtering now track cursor blink mode `?12`
  ...
  `config.cursor_blink_ease_out`, `config.default_cursor_style`, and
  `config.force_reverse_video_cursor` snippets now parse into the same native
  override path; richer cursor dimension Lua parsing remains open.
...
- Command palette now renders a visible native candidate overlay and honors the
  ...
  Static WezTerm-style Lua `config.command_palette_rows` and
...
- Native window now dispatches a typed `window-config-reloaded` hook for
  ...
  `status_update_interval`, `command_palette_rows`, `launcher_alphabet`,
  ...
  `font_size`, `cell_width`, `line_height`, `initial_cols`, `initial_rows`,
  ...
  `force_reverse_video_cursor`, plus
