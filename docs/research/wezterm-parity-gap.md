...
- Native window frame now defaults to a one-row tab bar with workspace/tab state,
  ...
  `show_new_tab_button_in_tab_bar`. Static Lua config parsing covers
...
- Command palette now exposes WezTerm-style `ReloadConfiguration`, and the
  ...
  native effective config. Static Lua config snippets for those five boolean
  fields parse into the same native override path. Unknown ESC/CSI sequences
...
- Native window now dispatches a typed `window-config-reloaded` hook for
  ...
  `hide_mouse_cursor_when_typing`, plus the implemented tab-bar boolean
  subset from `enable_tab_bar` through `show_tabs_in_tab_bar`, and the
  diagnostic/reload booleans `automatically_reload_config`,
  `use_resize_increments`, `debug_key_events`,
  `log_unknown_escape_sequences`, and `warn_about_missing_glyphs`.
