...
- App runtime and console output filtering now track kitty keyboard
  ...
  overrides the cursor color while Builtin preedit text or the leader modifier
  is active. Exact platform left/right modifier source tracking, dead-key
  composition cursor parity, deeper platform IME/XIM setup, and broader
  dynamic `cell_widths` Lua parity remain open.
...
- Native user `key_assignments` now dispatch matching regular key presses
  ...
  or the leader modifier is active. Exact platform left/right modifier source
  tracking, dead-key composition cursor parity, deeper platform IME/XIM setup,
