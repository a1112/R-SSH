...
- Native tab bar rendering now dispatches a typed `format-tab-title` hook after
  ...
  return `tab.tab_title`, `tab.window_title`, `tab.active_pane.title`, or
  `tab.active_pane.domain_name` resolve the current explicit tab title, window
  title, pane title, or pane domain name at render time; arbitrary Lua callbacks
