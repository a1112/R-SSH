...
- Native window state now exposes WezTerm-style `DecreaseFontSize`,
  ...
  `freetype_load_flags`, `initial_cols`, `initial_rows`, and
  ...
  at 100 DPI or higher. Actual renderer font rasterization changes and full
  Lua config evaluation remain open.
...
- Native window now dispatches a typed `window-config-reloaded` hook for
  ...
  `font_size`, `cell_width`, `line_height`, `font_antialias`, `font_hinting`, `font_rasterizer`, `freetype_load_target`, `freetype_render_target`, `freetype_load_flags`, `default_cursor_style`, `cursor_thickness`, `underline_thickness`, `underline_position`, `strikethrough_position`, `force_reverse_video_cursor`,
