...
- App runtime and console output filtering now track kitty keyboard
  ...
  take priority over that ambiguous-width setting. Native winit IME commit text
  ...
  cleared by commit or empty preedit. Exact platform left/right modifier source
  tracking, exact preedit cursor styling, deeper platform IME/XIM setup, and
  broader dynamic `cell_widths` Lua parity remain open.
...
- Native user `key_assignments` now dispatch matching regular key presses
  ...
  cursor and is suppressed for `System` or disabled IME. Exact platform
  left/right modifier source tracking, exact preedit cursor styling, deeper
  platform IME/XIM setup, broader dynamic `cell_widths` Lua parity, and broader
  kitty alternate-key variants remain open.
