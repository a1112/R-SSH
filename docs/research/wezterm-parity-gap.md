...
- Native window now dispatches a typed `format-window-title` hook after
  ...
  `tab.tab_id`, `tab.tab_title`, `tab.window_title`, callback-local
  dynamic title variables, or callback-local `tab.active_pane`/`pane`
  active-pane aliases resolve the active pane/tab id/title, window title, pane
  id, domain name, foreground process, current working directory, or tty name
  at recompute time, including `tostring(...)` wrappers;
