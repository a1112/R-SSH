...
- Native window now dispatches a typed `new-tab-button-click` hook for
  ...
  using `button == 'Left'/'Right'/'Middle'`,
  `button ~= 'Left'/'Right'/'Middle'`, or a callback-local alias assigned from
  `button` map onto the same suppression path. The
