...
- Static WezTerm-style Lua `config.color_schemes` entries can now define
  ...
  `Builtin Solarized Dark`, `Builtin Solarized Light`, and their WezTerm
  aliases using WezTerm's built-in TOML palette data. The broader built-in
  scheme catalog, richer dynamic `load_scheme` composition, and full dynamic
  Lua scheme construction remain later parity work.
