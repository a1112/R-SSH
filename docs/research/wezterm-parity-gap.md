...
- Native window state now exposes WezTerm-style `DecreaseFontSize`,
  ...
  interpreter-version, PCF long-family-name, and display pixel-geometry
  options are retained in effective config with WezTerm defaults, while actual
  renderer glyph strategy, FreeType interpreter application, subpixel geometry
  application, PCF font-resolution changes, and full Lua config evaluation
  remain open.
