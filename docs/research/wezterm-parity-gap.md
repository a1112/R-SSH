...
- Native window now dispatches typed `update-status` and deprecated
  ...
  `wezterm.on('update-status', function(window, pane) ... end)` callbacks with
  literal `window:set_left_status(...)` / `window:set_right_status(...)` calls
  update native tab-bar status text. Arbitrary Lua status callback execution,
