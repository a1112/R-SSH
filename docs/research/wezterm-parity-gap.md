...
- Command-palette close-current queries now accept both spaced forms
  ...
  variables for `confirm` plus parenthesized static options table variables
  when parsed from static WezTerm-style `config.keys`.
...
- Native `AttachDomain`/`DetachDomain` action parsing now recognizes official
  ...
  attach-domain strings and detach-domain `DomainName` fields, plus
...
- Command palette now exposes WezTerm-style `StartWindowDrag` as
  ...
  variables for action payloads, parsed mouse bindings are retained in native
...
- Native `EmitEvent` action payloads now carry a custom event name and dispatch
  ...
  variables for the table-call `name` field. Parenthesized `EmitEvent(event_opts)`
  calls also accept top-level static options table variables. Lua
...
- Native `ActivateKeyTable`, `PopKeyTable`, and `ClearKeyTableStack` action
  ...
  supported for nested inserts plus indexed or length-append assignments/field
  mutations where `static_name` resolves to `key_tables`. Static
...
- Native tab bar rendering now dispatches a typed `format-tab-title` hook after
  ...
  `wezterm.format` returns through static aliases map onto the same tab-title
  override path; arbitrary Lua callbacks and the full Lua config object remain
  open.
...
- Native window now dispatches typed `update-status` and deprecated
  ...
  `update-right-status` callbacks with literal `window:set_left_status(...)` /
  `window:set_right_status(...)` calls plus inline, static-table-variable, or
  static-alias `wezterm.format` Text/Foreground/Background/ResetAttributes and Attribute
  ...
  appends whose string items can resolve from static variables. Arbitrary Lua
  status callback execution, dynamic `wezterm.format` construction, and broader
  window status API wiring remain open.
...
- Native framebuffer rendering applies basic WezTerm-style
  ...
  Static `config.colors` scalar, ANSI/bright/indexed palette, selection, and
  ColorSpec fields now also accept `wezterm.color.parse(...)` calls, including
  top-level static aliases such as `local parse_color = wezterm.color.parse`.
  ...
  single `source = { Gradient = ... }` explicit color lists or preset names
