...
- App-shell state now exposes a named WezTerm-style `SwitchToWorkspace` subset
  ...
  default-program launch entries when `args` is omitted. Static
  ...
  dynamic Lua `launch_menu` construction, Lua `PromptInputLine` callback
  wiring, and Lua event/config wiring remain open.
...
- Static WezTerm-style Lua `config.color_schemes` entries can now define
  ...
  TOML path can also feed selected `config.color_schemes['Name']` entries, or
  `config.colors` directly or through the first returned variable from
  `local colors, metadata = ...` or `colors, metadata = ...`
