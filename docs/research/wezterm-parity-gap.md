...
- Native window now dispatches typed `update-status` and deprecated
  ...
  `pane:get_dimensions()` field concatenations for `cols`, `viewport_rows`,
  `scrollback_rows`, `physical_top`, and `scrollback_top`, static
  `pane:is_alt_screen_active()` status branches with static active/inactive
  strings, static `window:get_dimensions()` field concatenations for
  `pixel_width`, `pixel_height`, `dpi`, and
