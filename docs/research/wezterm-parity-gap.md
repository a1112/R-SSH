...
- App runtime and console output filtering now track kitty keyboard
  ...
  disabled, while exact platform left/right modifier source tracking, IME
  preedit rendering, deeper platform IME/XIM setup, and broader dynamic
...
- Native user `key_assignments` now dispatch matching regular key presses
  ...
  when `use_ime` is enabled and is ignored when disabled. Exact platform
  left/right modifier source tracking, IME preedit rendering, deeper platform
  IME/XIM setup, broader dynamic `cell_widths` Lua parity, and broader kitty
