...
- Native window now dispatches typed `update-status` and deprecated
  ...
  `window:set_right_status(...)` calls plus inline, callback-local, or top-level static
  ...
  text. Arbitrary Lua status callback execution, dynamic `wezterm.format`
