...
- Native tab bar rendering now dispatches a typed `format-tab-title` hook after
  ...
  concatenations, and direct or static-result-variable `wezterm.format` returns
  through static aliases map onto the same tab-title override path. Simple dynamic callbacks that
