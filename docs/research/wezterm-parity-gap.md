...
- Native window state now exposes WezTerm-style `StartWindowDrag`: the command
  ...
  appends, with `action` payloads inline or through top-level static action
...
- App-shell state now exposes a named WezTerm-style `SwitchToWorkspace` subset
  ...
  with bracket field selectors such as `config['launch_menu']` and static table
  variables such as `table.insert(config.launch_menu, item)` or
  `table.insert(config.launch_menu, index, item)` supported. Static
...
- Command palette now exposes WezTerm-style `StartWindowDrag` as
  ...
  including top-level static `table.insert(config.mouse_bindings, { ... })`
  appends and top-level static action variables for action payloads, and
...
- Native `ActivateKeyTable`, `PopKeyTable`, and `ClearKeyTableStack` action
  ...
  `user_keys[1] = { ... }` or `user_keys[#user_keys + 1] = { ... }` before the
  config assignment,
