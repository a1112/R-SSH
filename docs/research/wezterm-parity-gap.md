...
- Native window now dispatches a typed `new-tab-button-click` hook for
  ...
  static bool-variable returns and simple `if`/`elseif` bool-return branches
  using literal or static string-variable button names, plus callback-local
  alias chains assigned from `button`, map onto the same suppression path. The
