...
- Native framebuffer rendering applies basic WezTerm-style
  ...
  `integrated_title_button_color`, `window_frame` titlebar/button/border color
  fields, and ColorSpec fields plus static `config.colors` variable mutations
  now also accept `wezterm.color.parse(...)` calls, including top-level static
  aliases such as `local parse_color = wezterm.color.parse`.
