...
- Command palette now exposes WezTerm-style `ReloadConfiguration`, and the
  ...
  overrides for `font_size`, `cell_width`, `cell_widths`, `line_height`, `font_antialias`, `font_hinting`, `font_rasterizer`, `font_colr_rasterizer`, `font_shaper`, `harfbuzz_features`, `use_cap_height_to_scale_fallback_fonts`, `ignore_svg_fonts`, `sort_fallback_fonts_by_coverage`, `search_font_dirs_for_fallback`, `custom_block_glyphs`, `anti_alias_custom_block_glyphs`, `allow_square_glyphs_to_overflow_width`, `freetype_load_target`, `freetype_render_target`, `freetype_load_flags`, `freetype_interpreter_version`, `freetype_pcf_long_family_names`, `display_pixel_geometry`, `foreground_text_hsb`, `inactive_pane_hsb`, `text_background_opacity`, `window_background_opacity`, `kde_window_background_blur`, `macos_window_background_blur`, `win32_system_backdrop`, `win32_acrylic_accent_color`, `tab_max_width`, `status_update_interval`,
  ...
  `skip_close_confirmation_for_processes_named`, and `enable_tab_bar` remain
  open.
...
- Native window now dispatches a typed `window-config-reloaded` hook for
  ...
  `win32_acrylic_accent_color`, dispatches `window-config-reloaded`, and allows later
  ...
  `quit_when_all_windows_are_closed`, `window_close_confirmation`,
  `exit_behavior`, `clean_exit_codes`, `exit_behavior_messaging`,
  `skip_close_confirmation_for_processes_named`, and `enable_tab_bar`, and
  broader config option coverage remain open.
