...
- Native `CopyMode` assignment payloads now parse WezTerm-style
  ...
  route the implemented `Cell`, `Line`, and `Block` selection modes. Search-only
  CopyModeAssignment values and SetSelectionMode `Word`/`SemanticZone` remain
  open.
