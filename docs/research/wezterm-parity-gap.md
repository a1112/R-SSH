...
- Native tab bar rendering now dispatches a typed `format-tab-title` hook after
  ...
  FormatItem table returns map onto the same tab-title override path; arbitrary
  Lua callbacks and the full Lua config object remain open.
...
- Native window now dispatches typed `update-status` and deprecated
  ...
  `window:set_right_status(...)` calls plus inline or static-table-variable
