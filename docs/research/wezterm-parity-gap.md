...
- Native window now dispatches typed `update-status` and deprecated
  ...
  `window:set_right_status(...)` calls plus static `wezterm.format` Text-item
  visible-text composition update native tab-bar status text. Arbitrary Lua
  status callback execution, styled `wezterm.format` SGR composition, and
  broader window status API wiring remain open.
