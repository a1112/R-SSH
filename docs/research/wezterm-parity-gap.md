...
- Static WezTerm-style Lua `config.color_schemes` entries can now define
  ...
  name and custom `color_schemes` map are retained in effective config, and a
  WezTerm-style `resolved_palette` snapshot is derived from the final applied
  native color fields. Static
