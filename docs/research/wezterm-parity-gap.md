...
- Native tab bar rendering now dispatches a typed `format-tab-title` hook after
  ...
  inline/callback-local/top-level static FormatItem table returns, and direct
  or static-result-variable `wezterm.format` returns through static aliases
  map onto the same tab-title override path. Simple dynamic callbacks that
  ...
  at render time. Simple `..` concatenation returns can combine those dynamic
  title fields with literal/callback-local/top-level static string segments and
  `tab.active_pane.pane_id`, resolving the pane id as text; arbitrary Lua
  callbacks and the full Lua config object remain open.
- Native window now dispatches a typed `format-window-title` hook after
  ...
  aliases, map onto the native title override path. Simple dynamic callbacks
  that return `pane.title`, `tab.active_pane.title`, or `tab.tab_title`
  resolve the active pane/tab title at recompute time; arbitrary Lua callback
...
- Native window now dispatches typed `update-status` and deprecated
  ...
  `window:set_right_status(...)` arguments, plus inline,
