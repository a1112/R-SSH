...
- Native tab bar rendering now dispatches a typed `format-tab-title` hook after
  ...
  `tab.active_pane.domain_name`, `tab.active_pane.foreground_process_name`, or
  `tab.active_pane.current_working_dir` resolve the current explicit tab title,
  window title, pane title, pane domain name, pane foreground process name, or
  pane current working directory at render time; arbitrary Lua callbacks and the
