...
- Native window now dispatches a typed `format-window-title` hook after
  ...
  that return `pane.title` resolve the active pane title at recompute time;
  arbitrary Lua callback execution plus the full Lua config object remain open.
