...
- Native user `key_assignments` now dispatch matching regular key presses
  ...
  static exec domain `name`/`fixup_command`/label entries, and
  ...
  mux server/parser, daemon pid/log redirection, exec or serial domain
  connections, stat logger, or process rlimits remains open.
