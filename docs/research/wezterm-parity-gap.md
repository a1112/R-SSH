...
- Terminal core now records iTerm2/WezTerm inline image `OSC 1337;File=...`
  ...
  image-id encoding plus explicit row/column diacritics from Kitty's 0..255
  placeholder table render those virtual placements. Visible placement deletion
  keeps stored image data alive while a virtual placement still references that
  image; attempts to make a `U=1` virtual placement relative return `EINVAL`.
  Basic `a=d`
  ...
  remaining richer placement controls including placeholder diacritic
  inheritance, broader
  query responses beyond current direct/chunked direct payload and stored-image
