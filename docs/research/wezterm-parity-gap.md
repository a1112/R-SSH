...
| Config layer | Lua config, events, hot reload, plugins | File-backed startup/manual/automatic reload is implemented for the strict production grammar, including source rediscovery, last-known-good generations, per-window layering, and native debounced watching; arbitrary Lua VM/`require`, dynamic helpers, dependency/helper-file watch paths, registry-excluded fields, plugins, and general event execution remain open | ⚠ Partial |
...
- Native window state now exposes WezTerm-style `DecreaseFontSize`,
  ...
  comments inside the helper path or before the call payload, and static font
  value variables, retaining the primary family, fallback families, and
  supported font attributes in effective config.
...
- Native `EmitEvent` action payloads now carry a custom event name and dispatch
  ...
  or the equivalent `wezterm['emit'](...)` / `wezterm[<static-key>](...)`
  bracket-field forms re-enter matching static handlers, including through a
  top-level static
...
- Native `ActivateKeyTable`, `PopKeyTable`, and `ClearKeyTableStack` action
  ...
  Lua config evaluation and default key-table merging remain open; file-backed
  lifecycle reload applies only to the strict production grammar.
...
- Against pinned WezTerm
  `093bf6bf2b82b929ed80c04fd54ebc80464f715e`, native window startup now owns a
  file-backed lifecycle for the strict production grammar. It discovers
  explicit `--config-file`, `WEZTERM_CONFIG_FILE`, Windows portable
  executable-adjacent config, `$HOME/.wezterm.lua`,
  `$XDG_CONFIG_HOME/wezterm/wezterm.lua` or, when `XDG_CONFIG_HOME` is unset,
  `$HOME/.config/wezterm/wezterm.lua`, then Unix `$XDG_CONFIG_DIRS` candidates;
  `--skip-config` disables discovery, and repeated
  `--config name=value` values apply last above the file. Each startup, manual,
  or watcher attempt rediscovers the source. A successful initial load is
  consumed before the first PTY spawn as generation 1. A file
  read/parse/validation failure starts recoverable generation 0 defaults with a
  diagnostic, while an invalid CLI override fails before app construction.
  Derived `WEZTERM_CONFIG_FILE`/`WEZTERM_CONFIG_DIR` values are published only
  into child environments without mutating process-global environment state.
  Production parsing accepts the bounded direct static return-table or supported
  `wezterm.config_builder()` plus direct static-assignment form and explicitly
  rejects unsupported statements, dynamic values, and registry-excluded
  top-level/nested fields with source diagnostics. Arbitrary Lua VM execution,
  general `require`, dynamic helpers, and config dependency/helper-file watch
  paths remain open.
- Command palette `ReloadConfiguration`, action-name `reloadconfiguration`, and
  the default `Ctrl+Shift+R` shortcut now run one manager transaction across
  startup, pending, detached, and materialized apps. Success advances one base
  generation and applies it to every app before notifying each
  `window-config-reloaded` callback once; failure retains the last-known-good
  generation/effective state and latest diagnostic but still notifies every app
  once. Per-window overrides stay above the base layer, future pending/detached
  materialization inherits both layers, and existing PTYs are not respawned.
- One lifecycle-owned watcher uses the manager-owned base-plus-CLI
  `automatically_reload_config` value, never a per-window override. It watches
  each attempted source and eligible normalized parent, including failed
  attempted sources, coalesces create/modify/remove bursts with a fixed
  200 ms first-event debounce, and feeds the same reload transaction. Watched
  paths accumulate across rediscovery, including relative-file parent
  normalization for atomic replacement; the home file is watched but its noisy
  home parent is not. A later disabled generation adds no new paths, while the
  existing watcher and accumulated paths remain active.
  ...
  error window UI, arbitrary Lua config execution, dependency/helper-file
  watching, and broader Lua
  `window:set_config_overrides` wiring beyond static `update-status`
  overrides for `font_size`, `cell_width`, `cell_widths`, `line_height`, `font_antialias`, `font_hinting`, `font_rasterizer`, `font_colr_rasterizer`, `font_shaper`, `harfbuzz_features`, `font_dirs`, `font_locator`, `use_cap_height_to_scale_fallback_fonts`, `ignore_svg_fonts`, `sort_fallback_fonts_by_coverage`, `search_font_dirs_for_fallback`, `custom_block_glyphs`, `anti_alias_custom_block_glyphs`, `allow_square_glyphs_to_overflow_width`, `freetype_load_target`, `freetype_render_target`, `freetype_load_flags`, `freetype_interpreter_version`, `freetype_pcf_long_family_names`, `display_pixel_geometry`, `dpi`, `dpi_by_screen`, `webgpu_preferred_adapter`, `foreground_text_hsb`, `inactive_pane_hsb`, `text_background_opacity`, `window_background_opacity`, `kde_window_background_blur`, `macos_window_background_blur`, `win32_system_backdrop`, `win32_acrylic_accent_color`, `audible_bell`, `visual_bell`, `visual_bell_color`, `notification_handling`, `colors`, `color_scheme`, `color_schemes`, `color_scheme_dirs`, `tab_max_width`, `status_update_interval`,
  `window_decorations`, `integrated_title_buttons`, `integrated_title_button_alignment`, `integrated_title_button_color`, `integrated_title_button_style`, `initial_cols`, `initial_rows`,
  `adjust_window_size_when_changing_font_size`, `selection_word_boundary`,
  `default_prog`, `default_domain`, `default_workspace`,
  ...
  `skip_close_confirmation_for_processes_named`, `enable_tab_bar`, and
  platform input fields `key_map_preference`, `ui_key_cap_rendering`,
  `swap_backspace_and_delete`, `xcursor_theme`, `xcursor_size`,
  `palette_max_key_assigments_for_action`, `unicode_version`, `bidi_enabled`,
  `bidi_direction`, `ime_preedit_rendering`,
  `macos_forward_to_ime_modifier_mask`, and `xim_im_name` remain open.
...
- WezTerm-style quick-select table actions now accept parenthesized Lua table
  ...
  dot-call, or indexed action constructors, static `local <callback> =
  wezterm.action_callback` aliases plus module-alias/static-key
  `action_callback` aliases with Lua comments inside the dotted helper path or
  before the callback call, plus parenthesized calls that pass a top-level
...
- Native window now dispatches typed bell hooks with the window id and
  ...
  tables with trailing comma fields, and `colors.visual_bell`. Static
  `wezterm.on('bell', function(window, pane) ... end)` callbacks with literal
  or static status string `window:set_left_status(...)` /
  `window:set_right_status(...)` calls now update tab-bar status on BEL;
  arbitrary Lua bell callback execution remains open.
- Native window now dispatches a typed focus-change hook with the window id,
  ...
  / `window:set_right_status(...)` calls, including static
  `window:is_focused()` branches, now update tab-bar status on focus changes;
  arbitrary Lua focus callback execution remains open.
- Native window now dispatches a typed resize hook after successful terminal and
  ...
  window dimensions APIs. Static
  `wezterm.on('window-resized', function(window, pane) ... end)` callbacks with
  literal or static status string `window:set_left_status(...)` /
  `window:set_right_status(...)` calls, including static
  `window:get_dimensions()` reads, now update tab-bar status on resize;
...
- Native window now dispatches a typed `window-config-reloaded` hook for
  ...
  `default_workspace`, `prefer_to_spawn_tabs`, `set_environment_variables`,
  ...
  `window_background_images`, `window_background_layers`,
  `kde_window_background_blur`,
  `macos_window_background_blur`, `win32_system_backdrop`, and
  `win32_acrylic_accent_color`, plus bell/notification fields `audible_bell`,
  `visual_bell`, `colors.visual_bell`/`visual_bell_color`, and
  ...
  `palette_max_key_assigments_for_action`, `unicode_version`, `bidi_enabled`,
  `bidi_direction`, `ime_preedit_rendering`,
  `macos_forward_to_ime_modifier_mask`, and `xim_im_name`, plus domain fields
  `exec_domains`, `wsl_domains`, `unix_domains`, `ssh_domains`,
  `tls_servers`, `tls_clients`, and `serial_ports`, plus mux/resource fields
  ...
  `mux_env_remove`, `periodic_stat_logging`, `ulimit_nofile`, and
  `ulimit_nproc`, dispatches
  `window-config-reloaded`, and allows later
  `window:effective_config()` reads in the same callback to observe the
  override. A typed native
  ...
  `font_size`, `cell_width`, `cell_widths`, `line_height`, `font_antialias`, `font_hinting`, `font_rasterizer`, `font_colr_rasterizer`, `font_shaper`, `font_dirs`, `font_locator`, `use_cap_height_to_scale_fallback_fonts`, `ignore_svg_fonts`, `sort_fallback_fonts_by_coverage`, `search_font_dirs_for_fallback`, `custom_block_glyphs`, `anti_alias_custom_block_glyphs`, `allow_square_glyphs_to_overflow_width`, `freetype_load_target`, `freetype_render_target`, `freetype_load_flags`, `freetype_interpreter_version`, `freetype_pcf_long_family_names`, `display_pixel_geometry`, `dpi`, `dpi_by_screen`, `foreground_text_hsb`, `inactive_pane_hsb`, `text_background_opacity`, `window_background_opacity`, `background`, `window_background_image`, `window_background_image_hsb`, `window_background_gradient`, `window_background_images`, `window_background_layers`, `default_cursor_style`, `cursor_thickness`, `underline_thickness`, `underline_position`, `strikethrough_position`, `force_reverse_video_cursor`, `kde_window_background_blur`, `macos_window_background_blur`, `win32_system_backdrop`, `win32_acrylic_accent_color`, `window_frame`, `tab_bar_style`,
  `initial_cols`, `initial_rows`, `adjust_window_size_when_changing_font_size`,
  ...
  error window UI, arbitrary Lua config execution, dependency/helper-file
  watching, broader
  Lua `window:set_config_overrides` wiring beyond static `update-status`
  overrides for `font_size`, `cell_width`, `cell_widths`, `line_height`, `font_antialias`, `font_hinting`, `font_rasterizer`, `font_colr_rasterizer`, `font_shaper`, `harfbuzz_features`, `font_dirs`, `font_locator`, `use_cap_height_to_scale_fallback_fonts`, `ignore_svg_fonts`, `sort_fallback_fonts_by_coverage`, `search_font_dirs_for_fallback`, `custom_block_glyphs`, `anti_alias_custom_block_glyphs`, `allow_square_glyphs_to_overflow_width`, `freetype_load_target`, `freetype_render_target`, `freetype_load_flags`, `freetype_interpreter_version`, `freetype_pcf_long_family_names`, `display_pixel_geometry`, `dpi`, `dpi_by_screen`, `webgpu_preferred_adapter`, `foreground_text_hsb`, `inactive_pane_hsb`, `text_background_opacity`, `window_background_opacity`, `background`, `window_background_image`, `window_background_image_hsb`, `window_background_gradient`, `window_background_images`, `window_background_layers`, `kde_window_background_blur`, `macos_window_background_blur`, `win32_system_backdrop`, `win32_acrylic_accent_color`, `window_frame`, `tab_bar_style`, `audible_bell`, `visual_bell`, `visual_bell_color`, `notification_handling`, `colors`, `color_scheme`, `color_schemes`, `color_scheme_dirs`, `hyperlink_rules`, `tab_max_width`, `status_update_interval`,
  `window_decorations`, `integrated_title_buttons`, `integrated_title_button_alignment`, `integrated_title_button_color`, `integrated_title_button_style`, `initial_cols`, `initial_rows`,
  `adjust_window_size_when_changing_font_size`, `selection_word_boundary`,
  `default_prog`, `default_domain`, `default_workspace`,
  `prefer_to_spawn_tabs`, `set_environment_variables`,
  ...
  `mux_output_parser_buffer_size`, `mux_output_parser_coalesce_delay_ms`,
  `mux_env_remove`, `periodic_stat_logging`, `ulimit_nofile`, and
  `ulimit_nproc`, and platform
  input fields `key_map_preference`, `ui_key_cap_rendering`,
  `swap_backspace_and_delete`, `xcursor_theme`, `xcursor_size`,
  `palette_max_key_assigments_for_action`, `unicode_version`, `bidi_enabled`,
  `bidi_direction`, `ime_preedit_rendering`,
  `macos_forward_to_ime_modifier_mask`, and `xim_im_name`, and broader config
...
- Native tab bar rendering now dispatches a typed `format-tab-title` hook after
  ...
  honor WezTerm's first-handler-wins rule for multiple static registrations
  and support literal or top-level static string-variable/concatenated event names, literal string
  ...
  `tab.is_last_active`, `hover`, direct or callback-local active-pane alias
  `tab.active_pane.is_zoomed`, direct or callback-local active-pane alias
  `tab.active_pane.progress == 'Indeterminate'`, direct or callback-local
  active-pane alias `tab.active_pane.progress.Percentage ~= nil` /
  `tab.active_pane.progress.Percentage == nil`,
  callback-local progress variables assigned from those progress paths such as
  `progress == 'Indeterminate'`, `progress.Percentage ~= nil`, or
  `progress.Percentage == nil`,
  callback-local user-vars variables assigned from active-pane user-vars paths
  such as `vars['NAME'] ~= nil` or `vars['NAME'] == nil`,
  `#tabs > N`, or `#panes > N`
  conditions with post-`end` fallback returns or inline `else` fallback returns/assignments, plus
  ...
  aliases, self-referential title-variable concatenations, and title truncation
  assignments used by the documented elaborate tab-title example,
  ...
  return `tab.tab_id`, `tab.tab_index`, `#tabs`, `#panes`, `tab.tab_title`, `tab.window_title`, `tab.active_pane.title`,
  ...
  the current tab id/index, tab count, pane count, explicit tab title, window title, pane title, pane domain name,
  ...
  segments plus `tab.tab_id`, `tab.tab_index`, `#tabs`, `#panes`,
  `string.format` tab-index/tab-count number pairs, `tab.active_pane.pane_id`,
  and active-pane `tab.active_pane.user_vars.NAME` /
  `tab.active_pane.user_vars['NAME']` static-key values or callback-local
  user-vars variables assigned from those paths such as `vars.NAME` /
  `vars['NAME']`, plus
  direct or callback-local active-pane alias
  `tab.active_pane.progress.Percentage` / `tab.active_pane.progress.Error`
  values, or callback-local progress-variable `progress.Percentage` / `progress.Error`
  values, resolving the ids, index, counts, stored user vars, and active-pane
  progress percentage or error value as text;
...
- Native window now dispatches a typed `format-window-title` hook after
  ...
  `wezterm.on`/`wezterm['on']` aliases, module-alias `on` aliases,
  `require('wezterm').on` aliases, and aliases to static callback expressions,
  ...
  simple top-level `if`/`elseif` return branches using
  `tab.active_pane.is_zoomed`, `pane.is_zoomed`, callback-local active-pane
  aliases such as `active.is_zoomed`, direct or callback-local
  active-pane/user-vars aliases for `pane.user_vars.NAME ~= nil` /
  `pane.user_vars.NAME == nil`, direct or callback-local active-pane/progress
  aliases for `pane.progress.Percentage ~= nil` /
  `pane.progress.Percentage == nil` or `pane.progress == 'Indeterminate'`,
  `#tabs > N`, or `#panes > N`,
  plus top-level static
  ...
  assignments for `tab.active_pane.is_zoomed`, `#tabs > N`, and `#panes > N`
  with optional `elseif` and `else` fallback assignments, conditionally prepended/appended self-referential title-variable concatenations
  including `elseif`/`else` fallback assignments,
  ...
  return `pane.title`, `pane.pane_id`, `pane.domain_name`,
  `pane.foreground_process_name`, `pane.current_working_dir`, `pane.tty_name`,
  `pane.user_vars.NAME`, `pane.user_vars['NAME']`, `pane.progress.Percentage`,
  `pane.progress.Error`,
  `tab.active_pane.title`, `tab.active_pane.pane_id`,
  `tab.active_pane.domain_name`, `tab.active_pane.foreground_process_name`,
  `tab.active_pane.current_working_dir`, `tab.active_pane.tty_name`,
  `tab.active_pane.user_vars.NAME`, `tab.active_pane.user_vars['NAME']`,
  `tab.active_pane.progress.Percentage`, `tab.active_pane.progress.Error`,
  `tab.tab_id`, `tab.tab_index`, `tab.tab_index + N`, `tab.tab_title`,
  `tab.window_title`, `#tabs`, or `#panes`, callback-local
  dynamic title variables including inline `tab.tab_title` non-empty fallback
  to `tab.active_pane.title` through post-`end` fallback returns or inline
  `else` fallback returns, top-level helper fallback variables that return
  `tab_info.tab_title` when non-empty and otherwise
  `tab_info.active_pane.title`, callback-local user-vars variables such as
  `local vars = pane.user_vars`, callback-local progress variables such as
  `local progress = pane.progress`, or callback-local `tab.active_pane`/`pane`
  active-pane aliases resolve the active pane/tab id/index/title, window title,
  pane id, domain name, foreground process, current working directory, tty name,
  static-key user vars, progress percentage/error values, tab count, or
  active-tab pane count at recompute time, including `tostring(...)` wrappers;
...
- Native window now dispatches a typed `new-tab-button-click` hook for
  ...
  payload, Right click carries the default `ShowLauncher` action, and Middle
  click has no default action; returning `false` suppresses any default action.
  Static
  `wezterm.on('new-tab-button-click', function(...) return false end)` callbacks
  with omitted unused parameters plus literal or callback-local/top-level
  static bool-variable returns, direct button-comparison returns, and simple
  `if`/`elseif` bool-return branches using literal or static string-variable
  button names, plus callback-local alias chains assigned from `button`, map
  onto the same suppression path. The
  documented static callback shapes that call
  `window:perform_action(default_action, pane)` behind a truthy
  `default_action` guard, forward or reversed non-nil guard, or after a
  `default_action == nil` early-return guard before returning `false` run the
...
- Native window now dispatches a typed open-uri hook for ctrl-clicked OSC 8
  ...
  returns now suppress the default opener through the same hook path, and
  multiple static `open-uri` registrations are evaluated in registration order
  so non-matching prefix branches continue to later static handlers. The
...
- Native `CopyMode` assignment payloads now parse WezTerm-style
  ...
  full Lua config evaluation and default key-table merging remain open;
  file-backed lifecycle reload applies only to the strict production grammar.
...
- Native framebuffer rendering applies basic WezTerm-style
  ...
  through top-level static helper aliases, including module/static-key aliases
  such as `local gradient = wt[color_key][gradient_key]` or
  `local gradient_colors = wt[helper_key]`, for static inline gradient specs or
  ...
  inside helper paths used by static alias assignments.
  ...
  `local parse_color = wezterm.color.parse` and module/static-key aliases such
  as `local parse_color = wt[color_key][parse_key]`, with Lua comments allowed
  before the alias call payload, between direct dotted helper path segments, and
  inside the helper path used by static alias assignments.
...
- Static WezTerm-style Lua `config.color_schemes` entries can now define
  ...
  executable on Windows. Initial built-in scheme lookup covers `3024 (base16)`,
  ...
  `Atelier Lakeside Light (base16)`, `Atelier Plateau (base16)`,
  `Atelier Plateau Light (base16)`, `Atelier Savanna (base16)`,
  `Atelier Savanna Light (base16)`, `Atelier Seaside (base16)`,
  `Atelier Seaside Light (base16)`, `Atelier Sulphurpool (base16)`,
  ...
  `Bamboo Multiplex`, `Banana Blueberry`, `Batman`, `Belafonte Day`,
  ...
  `Black Metal (Khold) (base16)`, `Black Metal (Marduk) (base16)`,
  ...
  `Euphrasia (terminal.sexy)`, `Eva (base16)`, `Eva Dim (base16)`,
  ...
  `Later This Evening`, `Later This Evening (Gogh)`, `Lavandula`,
  `Lavandula (Gogh)`, `Light White (terminal.sexy)`,
  `Liquid Carbon (Gogh)`, `Liquid Carbon Transparent (Gogh)`,
  ...
  `MaterialOcean`, `Mathias`, `Mathias (Gogh)`, `matrix`,
  `Matrix (terminal.sexy)`, `Medallion`, `Medallion (Gogh)`,
  ...
  `Paraiso Dark`, `Paraiso Dark (Gogh)`,
  `Parker Brothers (terminal.sexy)`, `Pasque (base16)`,
  ...
  `Red Sands (Gogh)`, `Relaxed`, `Relaxed (Gogh)`, `Retro`,
  `Rezza (terminal.sexy)`, `Rippedcasts`, `Rippedcasts (Gogh)`,
  ...
  `Tomorrow (dark) (terminal.sexy)`, `Tomorrow (Gogh)`,
  `Tomorrow (light) (terminal.sexy)`, `Tomorrow Night`,
  ...
  `Twilight (dark) (terminal.sexy)`, `Twilight (Gogh)`,
...
- This bounded work does not claim complete selection parity, full App Shell
  v2, or general WezTerm parity. Richer pane focus visuals, arbitrary Lua
  callbacks, external CLI title control, and a real mux/window registry with
  domain, protocol, and renderer parity remain open.
- Native `enable_scroll_bar` defaults to false. When true, the scrollback
  ...
  applied to scrollbar rendering and hit-testing. In split layouts, the single
  scrollbar remains at the window's far-right edge and is bound to the active
  pane, so focus changes reflect that pane's independent history and scrollback
  offset while click/drag changes only the active pane. Pinned WezTerm currently
  uses the same single window-right active-pane scrollbar; per-pane scrollbar
  tracks remain an upstream TODO and are outside this slice. Static Lua
...
Beyond it, parity backlog remains richer pane focus visuals, arbitrary Lua
