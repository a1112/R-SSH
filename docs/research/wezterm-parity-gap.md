...
- Native window now dispatches a typed `format-window-title` hook after
  ...
  that return `pane.title` or `tab.tab_title` resolve the active pane/tab title
  at recompute time; arbitrary Lua callback execution plus the full Lua config
  object remain open.
