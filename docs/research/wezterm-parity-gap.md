...
- Native framebuffer rendering applies basic WezTerm-style
  ...
  orientation and explicit `colors` lists to default window backgrounds, with
  `Vertical` following WezTerm's bottom-to-top direction, Linear angles
  following WezTerm's counter-clockwise degree model, and Radial center/radius
  values following WezTerm's normalized window-space model. Static Lua snippets
  for that subset now parse into native/effective config. Presets,
  interpolation, blend modes, noise, and segment controls remain later parity
  work.
