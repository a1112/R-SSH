...
- Native window now dispatches a typed `format-window-title` hook after
  ...
  `pane.user_vars.NAME`, `pane.user_vars['NAME']`,
  ...
  static-key user vars, tab count, or active-tab pane count at recompute time,
  including `tostring(...)` wrappers;
