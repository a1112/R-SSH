...
- Native window now dispatches a typed `format-window-title` hook after
  ...
  return `pane.title`, `tab.active_pane.title`, `tab.tab_title`, callback-local
  dynamic title variables, or callback-local `tab.active_pane`/`pane`
  active-pane aliases resolve the active pane/tab title at recompute time,
