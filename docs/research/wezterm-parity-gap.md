...
- App runtime and console output filtering now track kitty keyboard
  ...
  config now retains `use_ime` with WezTerm's current default `true` plus
  optional `xim_im_name`; actual platform IME/XIM connection remains open.
...
- Native user `key_assignments` now dispatch matching regular key presses
  ...
  `config.use_ime`, and `config.xim_im_name` now parse into the same native
  override path; actual platform IME/XIM connection and broader kitty
...
- Native window now dispatches a typed `window-config-reloaded` hook for
  ...
  `enable_csi_u_key_encoding`, `enable_kitty_keyboard`, `allow_win32_input_mode`, `use_ime`, `xim_im_name`, `scroll_to_bottom_on_input`,
