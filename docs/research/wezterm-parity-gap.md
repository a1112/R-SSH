...
- Native window state now exposes WezTerm-style `DecreaseFontSize`,
  ...
  long-family-name, display pixel-geometry, font-directory, font-locator, font
  shaper, and fallback-font cap-height scaling options are retained in
  effective config with WezTerm defaults,
  while actual renderer glyph strategy, configured font-directory scanning,
  font-locator application, shaping-engine application, FreeType interpreter
  application, fallback-font cap-height scaling, subpixel geometry application,
  PCF font-resolution changes, and full Lua config evaluation remain open.
...
- App runtime and console output filtering now track kitty keyboard
  ...
  or a dead key is active. Broader dead-key expansion, deeper platform IME/XIM
  setup, and broader dynamic `cell_widths` Lua parity remain open.
...
- Native user `key_assignments` now dispatch matching regular key presses
  ...
  override while composition is pending. Deeper platform IME/XIM setup,
...
- Native window now dispatches a typed `window-config-reloaded` hook for
  ...
  `show_update_window`, `native_macos_fullscreen_mode`, `max_fps`, `animation_fps`, `use_resize_increments`, `debug_key_events`, and
  ...
  `text_background_opacity`, `window_background_opacity`, `kde_window_background_blur`,
  `macos_window_background_blur`, `win32_system_backdrop`,
  `integrated_title_buttons`, `integrated_title_button_alignment`,
...
- Native config retains WezTerm-style platform backdrop settings:
  ...
  `Acrylic`, `Mica`, and `Tabbed`. Static Lua snippets for those fields now
  parse into native/effective config. Applying the KDE Wayland blur protocol,
  macOS blur radius, or Windows backdrop APIs remains platform-adapter work.
