...
- Native terminal rendering applies WezTerm-style `colors.background` as the
  ...
  native override path for `#RRGGBB` and `rgb(r,g,b)` color strings;
  copy-mode, split, scrollbar, compose, tab-bar, and other non-terminal
...
- Native terminal rendering applies WezTerm-style `colors.foreground` as the
  ...
  path for `#RRGGBB` and `rgb(r,g,b)` color strings; copy-mode, split,
...
- Native terminal rendering applies WezTerm-style `colors.ansi` and
  ...
  snippets now parse into the same native override path for `#RRGGBB` and
  `rgb(r,g,b)` color strings.
- Native terminal rendering applies WezTerm-style `colors.indexed` entries for
  ...
  same native override path, including `rgb(r,g,b)` color strings; unspecified
  indexed entries continue to use the xterm 256-color cube/grayscale mapping.
- Native terminal selection rendering applies WezTerm-style
  ...
  path for `#RRGGBB` and `rgb(r,g,b)` colors, including
  `selection_fg = 'none'` preserving the current cell foreground, plus
  `selection_bg = 'rgba(r,g,b,a)'` alpha blending over the current cell
