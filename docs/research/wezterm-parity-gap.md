...
- Native tab bar rendering now dispatches a typed `format-tab-title` hook after
  ...
  `tab.is_last_active`, `hover`, `tab.active_pane.is_zoomed`, `#tabs > N`, or
  `#panes > N` conditions with post-`end` fallback returns or inline `else`
  fallback returns/assignments, plus
