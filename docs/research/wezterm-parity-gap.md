...
- Native window now dispatches a typed `window-config-reloaded` hook for
  ...
  `text_background_opacity`, `window_background_opacity`, `colors.foreground`, `colors.background`, `colors.ansi`, `colors.brights`, `colors.indexed`, `colors.selection_fg`, `colors.selection_bg`, `colors.cursor_bg`, `colors.cursor_border`, and `colors.cursor_fg`, plus
...
- Native terminal rendering applies WezTerm-style `colors.background` as the
  ...
  alpha, plus WezTerm `hsl:` and CSS-style `hsl(...)`/`hsla(...)`/`hsv(...)`
  color strings; copy-mode, split, scrollbar, compose, tab-bar, and other
...
- Native terminal rendering applies WezTerm-style `colors.foreground` as the
  ...
  `hsl:` and CSS-style `hsl(...)`/`hsla(...)`/`hsv(...)` color strings;
  copy-mode, split, scrollbar, compose, tab-bar, and other non-terminal color
...
- Native terminal rendering applies WezTerm-style `colors.ansi` and
  ...
  implemented CSS-style `rgb(...)`/`hsl(...)`/`hsla(...)`/`hsv(...)`
  color-string subset.
- Native terminal rendering applies WezTerm-style `colors.indexed` entries for
  ...
  `rgb(...)`/`hsl(...)`/`hsla(...)`/`hsv(...)` color-string subset;
  unspecified indexed entries continue to use the xterm 256-color
  cube/grayscale mapping.
- Native terminal selection rendering applies WezTerm-style
  ...
  path for `#RRGGBB` and the implemented CSS-style
  `rgb(...)`/`hsl(...)`/`hsla(...)`/`hsv(...)` subset, including slash alpha for
  `selection_bg`, `selection_fg = 'none'` preserving the current cell
  foreground, plus `selection_bg = 'rgba(r,g,b,a)'` and
  `selection_bg = 'hsla(h,s,l,a)'`/`hsv(h s v / a)` alpha blending over the
