...
- Native window state now exposes WezTerm-style `DecreaseFontSize`,
  ...
  renderer font rasterization and Lua config parsing remain open.
...
- App runtime and console output filtering now track kitty keyboard
  ...
  codes and control-state bits where the input backend exposes the side.
...
- App runtime and console output filtering now track cursor blink mode `?12`
  ...
  configured steady or blinking block/underline/bar default; Lua config
...
- App-shell state now exposes a named WezTerm-style `SwitchToWorkspace` subset
  ...
  silently discarding them. Remote/mux domains, richer default-mode UI styling,
  Lua key
  assignment/config parsing, Lua `ShowLauncherArgs`/`launch_menu` parsing,
  Lua `PromptInputLine` callback wiring, and Lua event/config wiring remain
  open.
...
- Command palette now renders a visible native candidate overlay and honors the
  ...
  visible row count is derived from terminal height.
- Quick-select labels now honor the WezTerm-style `quick_select_alphabet`
  ...
  set when true, and `quick_select_remove_styling` in native/effective config.
...
- Native `PromptInputLine` action payloads now carry `description`, optional
  ...
  subset. Lua `wezterm.action_callback` wiring remains open.
- Native `InputSelector` action payloads now carry `title`, `choices`, optional
  ...
  also dispatch that native field subset when `choices` uses either the existing
  semicolon-delimited string form or WezTerm's Lua table-of-tables choice form
  with `{ label = ..., id = ... }` entries. Lua `wezterm.action_callback`
  wiring remains open.
...
- Native `ActivateKeyTable`, `PopKeyTable`, and `ClearKeyTableStack` action
  ...
  including implemented `CopyMode` assignment payloads. Lua `key_tables`
  parsing remains open.
...
- Native user `key_assignments` now dispatch matching regular key presses
  ...
  modifier-key records where the input backend exposes the side; Lua config
  parsing remains open.
- Native `leader` overrides now expose a WezTerm-style `LEADER` modal modifier
  ...
  before normal input resumes. Lua `leader`/`keys` parsing remains open.
...
- Command palette now exposes WezTerm-style `ReloadConfiguration`, and the
  ...
  the native effective config. `use_resize_increments`,
  `debug_key_events`, and `log_unknown_escape_sequences` are retained with
  WezTerm's default `false` in the native effective config.
  `warn_about_missing_glyphs` is retained with WezTerm's default `true` in the
  native effective config. Unknown ESC/CSI sequences are recorded by the
  ...
  `warn_about_missing_glyphs` is enabled. Actual OS-specific resize increment
  application, full WezTerm-style configuration error window UI, Lua config
  reload, automatic file watching, and Lua `window:set_config_overrides` wiring
  remain open.
...
- App runtime now extracts WezTerm-documented OSC 9 and OSC 777 `notify`
  ...
  `Notification: ...`. Console output filtering also consumes OSC 9
...
- Native window now dispatches typed bell hooks with the window id and
  ...
  behavior. Lua event wiring and Lua config parsing remain open.
...
- Native window now dispatches a typed `window-config-reloaded` hook for
  ...
  `font_size`, `cell_width`, `line_height`, `default_cursor_style`, `cursor_thickness`, `underline_thickness`, `underline_position`, `strikethrough_position`, `force_reverse_video_cursor`,
  `initial_cols`, `initial_rows`, `adjust_window_size_when_changing_font_size`,
...
- Native window now dispatches typed `update-status` and deprecated
  ...
  `window:set_left_status` / `set_right_status` wiring remain open.
...
- Native `CopyMode` assignment payloads now parse WezTerm-style
  ...
  route the implemented `Cell`, `Line`, and `Block` selection modes. Search
  ...
  helpers as the default key table. Single-name Lua table forms such as
  `wezterm.action.CopyMode { 'ClearSelectionMode' }` now reuse the same
  assignment parser. Remaining edit and accept CopyModeAssignment values plus
  SetSelectionMode `Word`/`SemanticZone` remain open.
...
- Mouse word selection now honors WezTerm-style `selection_word_boundary`,
  including the documented default boundary set and per-window overrides.
...
- Native split rendering applies WezTerm-style `inactive_pane_hsb` defaults
  ...
  unchanged. Lua config parsing and dynamic palette-aware resolution for this
  option remain later parity work.
- Native terminal rendering applies WezTerm-style `foreground_text_hsb`
  ...
  while preserving background colors. Lua config parsing and dynamic
  palette-aware resolution for this option remain later parity work.
- Native terminal rendering applies WezTerm-style `bold_brightens_ansi_colors`
  ...
  supporting `No` plus `BrightOnly`. Lua config parsing and custom palette
  resolution remain later parity work.
- Native terminal rendering applies WezTerm-style `text_background_opacity`
  ...
  backgrounds. Lua config parsing and dynamic palette-aware resolution for this
  option remain later parity work.
- Native terminal rendering applies WezTerm-style `text_blink_rate`,
  ...
  Lua config parsing remains later parity work.
- Native terminal rendering applies WezTerm-style `underline_thickness`
  ...
  percent-of-default, and cell-fraction units. Lua config parsing,
  font-metric defaults, and split/custom-glyph line use remain later parity
  work.
- Native terminal rendering applies WezTerm-style `underline_position`
  ...
  underline-row baseline approximation. Lua config parsing and exact
  font-metric-derived baseline/default behavior remain later parity work.
- Native terminal rendering applies WezTerm-style `strikethrough_position`
  ...
  percent-of-default, and cell-fraction units. Lua config parsing and
...
- Native `enable_scroll_bar` defaults to false. When true, the scrollback
  ...
  scrollbar rendering and hit-testing; Lua config wiring remains open.
