...
- Native window now dispatches a typed `new-tab-button-click` hook for
  ...
  static bool-variable returns and simple `button == 'Left'/'Right'/'Middle'`
  or `button ~= 'Left'/'Right'/'Middle'` bool-return branches map onto the
  same suppression path. The
