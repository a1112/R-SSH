...
- Command palette now exposes WezTerm-style `ReloadConfiguration`, and the
  ...
  `prefer_to_spawn_tabs`, `set_environment_variables`, and `enable_tab_bar`
  remain open.
...
- Native window now dispatches a typed `window-config-reloaded` hook for
  ...
  `set_environment_variables`, and `enable_tab_bar` overrides into the native
  effective config, dispatch
  ...
  overrides for `font_size`, `tab_max_width`, `status_update_interval`,
  `default_workspace`, and `enable_tab_bar`, and broader config option coverage
  remain open.
