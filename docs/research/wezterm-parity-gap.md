...
- Native window now dispatches typed bell hooks with the window id and
  ...
  `config.colors.cursor_bg`, `config.colors.cursor_fg`, and
...
- Native window now dispatches a typed `window-config-reloaded` hook for
  ...
  `term`, `audible_bell`, `visual_bell`, `foreground_color`, `background_color`, `cursor_bg_color`, `cursor_fg_color`, `visual_bell_color`, `notification_handling`, `default_prog`, `default_domain`, `default_workspace`, `automatically_reload_config`, `use_resize_increments`, `debug_key_events`, `log_unknown_escape_sequences`, `warn_about_missing_glyphs`, `default_cwd`, `set_environment_variables`,
  ...
  `audible_bell`, `visual_bell`, `colors.foreground`, `colors.background`, `colors.cursor_bg`, `colors.cursor_fg`, `colors.visual_bell`, and
  ...
  `text_background_opacity`, `window_background_opacity`, `colors.foreground`, `colors.background`, `colors.cursor_bg`, and `colors.cursor_fg`, plus
...
- Native terminal rendering applies WezTerm-style `colors.foreground` as the
  ...
  path; broader palette-table coverage remains later parity work.
- Native cursor rendering applies WezTerm-style `colors.cursor_bg` as the
  default cursor color and `colors.cursor_fg` as block-cursor text foreground
  unless OSC cursor color or `force_reverse_video_cursor` takes precedence for
  cursor background behavior. Static WezTerm-style Lua
  `config.colors.cursor_bg` and `config.colors.cursor_fg` snippets now parse
  into the same native override path; `cursor_border` remains later parity
  work.
