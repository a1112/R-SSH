...
- Native terminal selection rendering applies WezTerm-style
  ...
  and `quick_select_label_bg`/`quick_select_label_fg` plus
  `quick_select_match_bg`/`quick_select_match_fg` apply to quick-select label
  and match rendering. Copy-mode highlight colors remain later overlay parity
  work.
- Native cursor rendering applies WezTerm-style `colors.cursor_bg` as the
  ...
  quick-select label/match colors apply to overlay rendering, while copy-mode
  highlights, split, scrollbar, compose, tab-bar, and other non-terminal color
