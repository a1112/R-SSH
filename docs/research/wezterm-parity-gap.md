...
- Native window state now exposes WezTerm-style `DecreaseFontSize`,
  ...
  application, subpixel geometry application, PCF font-resolution changes, and
  full Lua config evaluation remain open.
...
- Native `SpawnTab` action payloads now carry a local-domain subset:
  ...
  fields. Remote/mux named domain spawning remains open.
- Native `AttachDomain`/`DetachDomain` action parsing now recognizes official
  ...
  attach-domain strings and detach-domain `DomainName` fields. Because the
