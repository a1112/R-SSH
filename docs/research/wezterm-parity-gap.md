...
- Command palette now exposes WezTerm-style `ReloadConfiguration`, and the
  ...
  `font_size` overrides remain open.
...
- Native window now dispatches a typed `window-config-reloaded` hook for
  ...
  `window:set_config_overrides { font_size = ... }` tables now merge the font
  size override into the native effective config, dispatch
  `window-config-reloaded`, and allow later
  `window:effective_config().font_size` reads in the same callback to observe
  the override. A typed native
  ...
  `font_size` overrides, and broader config option coverage remain open.
