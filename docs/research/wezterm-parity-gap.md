...
- Native window now dispatches a typed `window-config-reloaded` hook for
  ...
  `palette_max_key_assigments_for_action`, `unicode_version`, `bidi_enabled`,
  `bidi_direction`, `ime_preedit_rendering`,
  `macos_forward_to_ime_modifier_mask`, and `xim_im_name`, plus mux/resource
  fields `default_mux_server_domain`,
  ...
  `quit_when_all_windows_are_closed`, `window_close_confirmation`,
  `exit_behavior`, `clean_exit_codes`, `exit_behavior_messaging`,
  `skip_close_confirmation_for_processes_named`, `enable_tab_bar`, mux/resource
  fields `default_mux_server_domain`,
  `ratelimit_mux_line_prefetches_per_second`,
  `mux_output_parser_buffer_size`, `mux_output_parser_coalesce_delay_ms`,
  `mux_env_remove`, `periodic_stat_logging`, `ulimit_nofile`, and
