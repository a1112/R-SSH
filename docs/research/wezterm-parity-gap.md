...
- Native tab bar rendering now dispatches a typed `format-tab-title` hook after
  ...
  `tab.active_pane.domain_name`, or `tab.active_pane.foreground_process_name`
  resolve the current explicit tab title, window title, pane title, pane domain
  name, or pane foreground process name at render time; arbitrary Lua callbacks
  and the full Lua config object remain open.
