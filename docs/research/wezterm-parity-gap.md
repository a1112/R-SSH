...
| Config layer | Lua config, events, hot reload, plugins | File-backed startup/manual/automatic reload is implemented for the strict production static grammar, including source rediscovery, last-known-good generations, per-window layering, and native debounced watching; arbitrary Lua VM/`require`, dynamic helpers, dependency/helper-file watch paths, registry-excluded fields, plugins, and general event execution remain open | ⚠ Partial |
...
- Against pinned WezTerm
  ...
  file-backed lifecycle for the strict production static grammar. It discovers
  explicit `--config-file`, `WEZTERM_CONFIG_FILE`, Windows portable
  executable-adjacent, the home config, XDG config-home fallback, then Unix XDG
  config-directory candidates; `--skip-config` disables discovery, and repeated
