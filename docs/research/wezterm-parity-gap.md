...
- Command palette now exposes WezTerm-style `ReloadConfiguration`, and the
  ...
  `default_workspace`, and `enable_tab_bar` remain open.
...
- Native window now dispatches a typed `window-config-reloaded` hook for
  ...
  `tab_max_width`, `status_update_interval`, `default_workspace`, and
  `enable_tab_bar` overrides into the native effective config, dispatch
