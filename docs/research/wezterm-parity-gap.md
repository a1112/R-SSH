...
- Native window now dispatches a typed `format-window-title` hook after
  ...
  `tab.active_pane.domain_name`, `tab.active_pane.foreground_process_name`,
  `tab.active_pane.current_working_dir`, `tab.tab_title`, callback-local
  ...
  foreground process, or current working directory at recompute time, including
  `tostring(...)` wrappers;
