...
- Native framebuffer rendering applies basic WezTerm-style
  ...
  `source = { Gradient = ... }` explicit color lists into the native window
  background gradient path with layer opacity applied to gradient color alpha.
  Layer `hsb` tables apply inline or through top-level static table variables
  to Color sources and explicit Gradient color lists.
  Multi-layer composition, image sources,
  alignment/repeat settings, and HSB/opacity transforms for preset/image sources
