...
- Native framebuffer rendering applies basic WezTerm-style
  `window_background_gradient` tables with `Horizontal`, `Vertical`, or
  `Linear = { angle = ... }` orientation and explicit `colors` lists to default
  window backgrounds, with `Vertical` following WezTerm's bottom-to-top
  direction and Linear angles following WezTerm's counter-clockwise degree
  model. Static Lua snippets for that subset now parse into native/effective
  config. Radial gradients, presets, interpolation, blend modes, noise, and segment
  controls remain later parity work.
