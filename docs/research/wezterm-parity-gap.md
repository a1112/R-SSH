...
- Native framebuffer rendering applies basic WezTerm-style
  ...
  opaque `source = { Gradient = ... }` into the native window background
  gradient path. Layer `hsb` tables apply inline or through top-level static
  table variables to Color sources and explicit Gradient color lists.
  Multi-layer composition, image sources,
  alignment/repeat settings, HSB transforms for preset/image sources, and
  semi-transparent gradient layers remain future parity work.
