...
- Terminal core now base64-decodes iTerm2/WezTerm `OSC 1337;SetUserVar`
  ...
  static string keys, and static string-key variables from the same stored
...
- Native window now dispatches typed `update-status` and deprecated
  ...
  static string-key-variable concatenations for stored user-var names,
  including static `or` fallbacks,
